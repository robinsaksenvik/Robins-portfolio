...
export default function About() {
  return (
    <div className="space-y-12">
      ...
      <section className="grid gap-8 rounded-3xl border border-foreground/30 bg-card p-10 shadow-[6px_6px_0_0_hsl(var(--foreground)/0.55)] lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] relative z-10">
        <div className="space-y-6">
          ...
          <div className="grid gap-3 text-sm uppercase tracking-[0.25em] text-foreground/70 md:grid-cols-2">
            <div className="rounded-2xl border border-foreground/30 bg-secondary px-5 py-4 shadow-[3px_3px_0_0_hsl(var(--foreground)/0.4)]">
              Experimental design
            </div>
            <div className="rounded-2xl border border-foreground/30 bg-secondary px-5 py-4 shadow-[3px_3px_0_0_hsl(var(--foreground)/0.4)]">
              User focus
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3 text-xs uppercase tracking-[0.3em] text-foreground/70">
            {skills.map((skill) => (
              <span
                key={skill}
                className="rounded-lg border border-foreground/30 bg-background px-4 py-2 shadow-[3px_3px_0_0_hsl(var(--foreground)/0.4)] relative z-10"
              >
                {skill}
              </span>
            ))}
      ...
      <section className="rounded-3xl border border-foreground/30 bg-secondary p-10 shadow-[6px_6px_0_0_hsl(var(--foreground)/0.5)] relative z-10">
        <h2 className="text-2xl font-heading font-bold uppercase tracking-[0.25em]">Timeline</h2>
        <div className="mt-6 space-y-5">
          {timeline.map((item) => (
            <div
              key={item.year}
              className="flex flex-col gap-2 rounded-2xl border border-foreground/30 bg-background px-5 py-4 shadow-[3px_3px_0_0_hsl(var(--foreground)/0.45)] md:flex-row md:items-start md:justify-between relative z-10"
            >
